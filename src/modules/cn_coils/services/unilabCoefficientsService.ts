/**
 * UNILAB Coefficients Service — lazy loader + normalization + fan curve evaluator.
 *
...
export interface UnilabCoefficientsBundle {
...
const URL = "/data/catalogs/unilabCoefficients.json";
...
export async function loadUnilabCoefficients(): Promise<UnilabCoefficientsBundle> {
  ...
  _inflight = (async () => {
    ...
    if (!res.ok) {
      throw new Error(
        `Falha ao carregar unilabCoefficients.json (HTTP ${res.status})`,
      );
    }
    const data = (await res.json()) as UnilabCoefficientsBundle;
    _cache = data;
    return data;
  })();
  try {
...
export function buildFanAudit(bundle: UnilabCoefficientsBundle): FanAuditSummary {
...
export function listUsableAxialFans(
  bundle: UnilabCoefficientsBundle,
): AxialFanRecord[] {
