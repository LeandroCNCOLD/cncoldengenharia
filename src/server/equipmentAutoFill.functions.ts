...
export type AutoFillItemPreview = {
  ...
  label: string;
  /** Nome humano do que será criado/vinculado. */
  description: string;
  /** True se já existe componente/link daquela role. */
  conflict: boolean;
  /** Detalhe do conflito existente. */
  existing: { kind: string; label: string } | null;
  /** Disponibilidade do dado para preencher. */
  available: boolean;
...
// ============================================================================
// Helpers (parsing raw_json) — reaproveitam lógica do coilUnilab
// ============================================================================
...
function tryGetFromCurvaRaw(raw: Record<string, unknown>, key: string): string | undefined {
...
function extractGeometry(raw: Record<string, unknown>, side: "evaporador" | "condensador"): CoilGeometry {
...
function extractMidPointOps(curva: unknown): {
  ...
  if (arr.length === 0) {
    return { tempEvapC: null, tempCondC: null, capacityKcalh: null, airflowM3h: null, rhInPct: null, superheatK: null, subcoolingK: null };
  }
